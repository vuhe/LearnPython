...
def inference(input_tensor, train, regularizer):
    ...
    with tf.compat.v1.variable_scope('layer1-conv1'):
        conv1_weight = tf.compat.v1.get_variable("weight", [CONV1_SIZE, CONV1_SIZE, NUM_CHANNELS, CONV1_DEEP],
                                                 initializer=tf.compat.v1.truncated_normal_initializer(stddev=0.1))
        conv1_biases = tf.compat.v1.get_variable("bias", [CONV1_DEEP],
                                                 initializer=tf.compat.v1.constant_initializer(0.0))

        # 使用边长为5，深度为32的过滤器，过滤器移动的步长为1，且使用全0填充
        conv1 = tf.nn.conv2d(input=input_tensor, filters=conv1_weight, strides=[1, 1, 1, 1], padding='SAME')
    ...
    with tf.compat.v1.name_scope('layer2-pool1'):
        pool1 = tf.nn.max_pool2d(input=relu1, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')

    # 声明第三层卷积层前向传播过程，这一层输入为14*14*32，输出为14*14*64
    with tf.compat.v1.variable_scope('layer3-conv2'):
        conv2_weight = tf.compat.v1.get_variable("weight", [CONV2_SIZE, CONV2_SIZE, CONV1_DEEP, CONV2_DEEP],
                                                 initializer=tf.compat.v1.truncated_normal_initializer(stddev=0.1))
        conv2_biases = tf.compat.v1.get_variable("bias", [CONV2_DEEP],
                                                 initializer=tf.compat.v1.constant_initializer(0.0))

        # 使用边长为5，深度为64的过滤器，步长为1，全0填充
        conv2 = tf.nn.conv2d(input=pool1, filters=conv2_weight, strides=[1, 1, 1, 1], padding='SAME')
        relu2 = tf.nn.relu(tf.nn.bias_add(conv2, conv2_biases))

    # 实现第四层池化层的前向传播过程，与第二层结构一样，输入为14*14*64，输出为7*7*64
    with tf.compat.v1.name_scope('layer4-pool2'):
        pool2 = tf.nn.max_pool2d(input=relu2, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
    ...
    with tf.compat.v1.variable_scope('layer5-fc1'):
        fc1_weight = tf.compat.v1.get_variable("weight", [nodes, FC_SIZE],
                                               initializer=tf.compat.v1.truncated_normal_initializer(stddev=0.1))
        # 只有全连接层的权重需要加入正则化
        if regularizer is not None:
            tf.compat.v1.add_to_collection('losses', regularizer(fc1_weight))
        fc1_biases = tf.compat.v1.get_variable("bias", [FC_SIZE], initializer=tf.compat.v1.constant_initializer(0.1))
        fc1 = tf.nn.relu(tf.matmul(reshaped, fc1_weight) + fc1_biases)
        if train:
            fc1 = tf.nn.dropout(fc1, rate=1 - 0.5)  # 0.5为每个元素被保存下来的概率

    # 声明第六层全连接层的前向传播过程，这一层的输入为长度是512的向量，输出为10的一维向量
    # 结果需要通过softmax层
    with tf.compat.v1.variable_scope('layer6-fc2'):
        fc2_weight = tf.compat.v1.get_variable("weight", [FC_SIZE, OUTPUT_NODE],
                                               initializer=tf.compat.v1.truncated_normal_initializer(stddev=0.1))
        if regularizer is not None:
            tf.compat.v1.add_to_collection('losses', regularizer(fc2_weight))
        fc2_biases = tf.compat.v1.get_variable("bias", [OUTPUT_NODE],
